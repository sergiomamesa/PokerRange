    ...
    public class FileReaderFactory
    {
        public FileReader CreateInstance(FileReaderFactoryParams parameters)
        {
            ...
            if (parameters.Situation == SituationType.FacingRaise)
                return new FileReader(String.Format("{0}\\FacingRaise\\{1}_{2}.csv", filesPath, parameters.HeroPosition.ToString(), parameters.VillainPosition.ToString()));

            if (parameters.Situation == SituationType.RFIvs3Bet)
                return new FileReader(String.Format("{0}\\RFIvs3Bet\\{1}_{2}.csv", filesPath, parameters.HeroPosition.ToString(), parameters.VillainPosition.ToString()));
